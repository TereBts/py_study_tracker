...
class Command(BaseCommand):
    ...
    Attributes:
        help (str): Short description shown in the Django 'help' command
        output.
    """
    ...
    def add_arguments(self, parser):
        ...
        Options:
            ...
            --clean (bool): If set, delete previously seeded GoalOutcome
            rows first.
        """
    ...
    def handle(self, *args, **opts):
        ...
        Generates synthetic GoalOutcome entries for a configurable number of
        past weeks.
        Each GoalOutcome includes randomized but plausible 'hours_completed'
        and 'lessons_completed' values, as well as completion flags based on
        goal targets.
            ...
            for i in range(weeks, 0, -1):
                ...
                if g.weekly_lessons_target:
                    lessons = max(
                        ...
                        int(
                            round(
                                g.weekly_lessons_target * random.uniform(
                                    0.6,
                                    1.4
                                )
                            )
                ...
                # Mark as completed if either target threshold is met or
                # exceeded
                completed = False
                if (
                    g.weekly_hours_target
                    and hours >= g.weekly_hours_target
                ):
                    completed = True
                if (
                    g.weekly_lessons_target
                    and lessons >= g.weekly_lessons_target
                ):
                    completed = True
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Seeded fake history for "
                f"{goals.count()} goals over {weeks} weeks."
            )
        )
