...
def last_week_range(today: Optional[date] = None) -> Tuple[date, date]:
    ...
    If `today` is omitted, uses the local date from Django's timezone utilities.

    Args:
        today (Optional[date]): Anchor date to calculate from. Defaults to local today.
...
def freeze_weekly_outcomes(
    ...
    """
    Summarise a week's study activity into GoalOutcome snapshots for active goals.
    ...
    For each active Goal, it:
      - Sums StudySession.duration_minutes for the week and converts to hours (1 dp).
      - Approximates lessons_completed via session count (if one session ≈ one lesson).
      - Marks `completed=True` if either weekly_hours_target or weekly_lessons_target is met.
      - Upserts (update_or_create) a GoalOutcome for (goal, week_start).

    Args:
        week_start (Optional[date]): Monday of the week to freeze. If None, inferred.
        week_end (Optional[date]): Sunday of the week to freeze. If None, inferred.
        dry_run (bool): If True, prints what would be written without touching the DB.

    Returns:
        dict: A summary with keys:
            - "created" (int): Number of GoalOutcome rows created.
            - "updated" (int): Number of GoalOutcome rows updated.
            - "week_start" (date | None): The effective Monday used (or None if skipped).
            - "week_end" (date | None): The effective Sunday used (or None if skipped).
    """
